  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden">
              ...
              {filtered.map((c) => (
                <tr key={c.company} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-3 text-right text-sm font-medium text-slate-700">
                    {c.est_value ? `AED ${(c.est_value / 1000).toFixed(0)}K` : '-'}
                  </td>
                  <td className="px-4 py-3">
                    <span className="badge bg-slate-100 text-slate-600">{c.lead_source}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {filtered.length === 0 && (
          <div className="px-6 py-12 text-center text-sm text-slate-400">No companies match your filters</div>
        )}
      </div>
    </div>
  );
